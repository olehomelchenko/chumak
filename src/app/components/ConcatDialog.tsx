...
import * as ConcatHandlers from '../handlers/concat-handlers';
...
export function ConcatDialog() {
  const { targetModel, targets, previewData, previewError, isPreviewing } = DialogStore.concatState;

  const handleTargetChange = (e: Event) => {
    const target = e.target as HTMLSelectElement;
    targetModel.value = target.value || null;
    ConcatHandlers.onConcatTargetChange();
  };

  const handlePreview = () => {
    ConcatHandlers.previewConcat();
  };
  ...
  return (
    <div>
      ...
      <div class={styles.group}>
        <label class={styles.label} for="concat-target-select">
          Concat With
        </label>
        <select
          id="concat-target-select"
          class={styles.input}
        ...
        <div class={styles.helpText}>
          Stacks rows from the selected model/source below current rows (keeps duplicates)
        </div>
      </div>
      ...
      <div class={styles.group}>
        ...
        >
          {isPreviewing.value ? 'Previewing...' : 'Preview Concat'}
        </button>
      </div>
